...
void test('starts the packaged Codex process and invokes a representative command', async () => {
  ...
  try {
    const processPath = fileURLToPath(
      new URL('../../extension/node/dist/codexProcess.js', import.meta.url),
    )
    ...
    await rpc.invoke('Codex.test.useMockData', {
      threads: [
        {
    ...
    const sessions = await rpc.invoke('Codex.listSessions')
