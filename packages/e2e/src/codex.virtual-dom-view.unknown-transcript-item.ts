...
export const name = 'codex.virtual-dom-view.unknown-transcript-item'
...
export const test: Test = async ({ Command, expect, Locator }) => {
    ...
    turns: [
      {
        ...
        error: null,
        id: 'turn-activity',
        items: [{ id: 'activity-1', type: 'fileChange' }],
  ...
  await expect(label).toHaveText('Activity')
  await expect(text).toHaveText('file Change')
}
